...
| 📝 **Итерация TD-2** | Улучшение типизации | P0 | ⏳ Ожидает | - | - |
| ♻️ **Итерация TD-3** | Рефакторинг MessageHandler | P1 | ⏳ Ожидает | - | - |
...
- [ ] **src/bot/message_handler.py**:
  - [ ] Убрать `Optional` из `__init__` для `llm_client` и `conversation`
  - [ ] Сделать их обязательными параметрами
  - [ ] Удалить fallback echo-режим в `handle_text` (строки 121-126)
  - [ ] Обновить docstring для `__init__`
- [ ] **src/llm/llm_client.py**:
  - [ ] Заменить `List[Dict[str, str]]` на `list[dict[str, str]]` (PEP 585)
  - [ ] Изменить сигнатуру `get_response`: `history: list[dict[str, str]] | None = None`
- [ ] **src/llm/conversation.py**:
  - [ ] Заменить `Dict[int, List[Dict[str, str]]]` на `dict[int, list[dict[str, str]]]`
  - [ ] Обновить все type hints на современный синтаксис
- [ ] **src/config/settings.py**:
  - [ ] Добавить константу `telegram_message_max_length: int = 4000`
- [ ] Запустить приложение через `make run` и проверить работу
- [ ] Проверить соответствие **conventions.mdc** (строгая типизация)
- [ ] Проверить соответствие **vision.md** (явное > неявного)
- [ ] **Обновить `.cursor/rules/workflow.mdc`**:
  - [ ] Добавить примеры современного синтаксиса типов (PEP 585) в шаблоны кода
  - [ ] Обновить чеклист с требованием использования современных type hints
...
- [ ] **src/bot/message_handler.py**:
  - [ ] Создать приватный метод `_split_message(text: str, max_length: int) -> list[str]`
  - [ ] Переместить логику разбивки из `handle_text` в новый метод (строки 145-176)
  - [ ] Добавить docstring для `_split_message` с примерами
  - [ ] Упростить `handle_text`, используя `_split_message`
  - [ ] Использовать константу из Settings для `max_length`
- [ ] Добавить логирование в `_split_message` при разбивке на части
- [ ] Запустить приложение и протестировать с длинным ответом (>4000 символов)
- [ ] Проверить соответствие **conventions.mdc** (приватные методы с `_`)
- [ ] Проверить соответствие **vision.md** (читаемость кода)
- [ ] **Обновить `.cursor/rules/workflow.mdc`**:
  - [ ] Добавить в шаблон согласования примеры выделения приватных методов
  - [ ] Обновить примеры с рефакторингом для улучшения читаемости
